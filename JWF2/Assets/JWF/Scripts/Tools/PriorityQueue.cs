...
namespace VCSKicksCollection
{
		...
		where T : IComparable<T>
	{
		...
		public List<T> Values { get { return storedValues; } }
		...
		{
			if (this.Count == 0 || index < 1 || index >= this.storedValues.Count)
			{
				return default(T);
			}

			T value = this.storedValues[index];

			if (index == this.storedValues.Count - 1)
			{
				storedValues.RemoveAt(index);
			}
			else if (this.storedValues.Count > 2)
			{
				T lastValue = this.storedValues[storedValues.Count - 1];
